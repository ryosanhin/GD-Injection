...
const OrderedInjectionTarget := preload(
	"res://tests/fixtures/scopes/ordered_injection_target.gd"
)
...
func _test_root_scope_initialization_async() -> void:
	...
	var scene_scope: TestContainerScope = BasicScopeScene.instantiate()
...
func _test_parent_initializes_before_child_async() -> void:
	...
	var parent: TestContainerScope = pair.get_node("Parent")
	root.add_child(pair)
